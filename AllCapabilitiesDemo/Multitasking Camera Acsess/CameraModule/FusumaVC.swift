...
public protocol FusumaDelegate1: class {
...
public extension FusumaDelegate1 {
    func fusumaImageSelected(_ image: UIImage, source: FusumaMode1, metaData: ImageMetadata1) {}
    func fusumaMultipleImageSelected(_ images: [UIImage], source: FusumaMode1, metaData: [ImageMetadata1]) {}
    func fusumaDismissedWithImage(_ image: UIImage, source: FusumaMode1) {}
    func fusumaClosed() {}
...
public var autoDismissFusu: Bool = true

@objc public enum FusumaMode1: Int {
    case camera
    case library
    case video

    static var all: [FusumaMode1] {
        return [.camera, .library, .video]
    }
}

public struct ImageMetadata1 {
...
@objc public class FusumaVC: UIViewController {
    ...
    private var mode: FusumaMode1 = .camera

    public var availableModes: [FusumaMode1] = [.library, .camera]
    ...
    lazy var cameraView = FSCameraViewViewController.instance()
    lazy var videoView  = FSVideoCameraViewVC.instance()

    fileprivate var hasGalleryPermission: Bool {
        return PHPhotoLibrary.authorizationStatus() == .authorized
    }

    public weak var delegate: FusumaDelegate1? = nil

    override public func loadView() {
        if let view = UINib(nibName: "FusumaVC", bundle: Bundle(for: self.classForCoder)).instantiate(withOwner: self, options: nil).first as? UIView {
    ...
    override public func viewDidLoad() {
        ...
        self.view.backgroundColor = UIColor.black//fusumaBackgroundColor
        ...
        menuView.backgroundColor = .clear//fusumaBackgroundColor
//        menuView.addBottomBorder(UIColor.black, width: 1.0)
        ...
        let closeImage = fusumaCloseImage != nil ? fusumaCloseImage : UIImage(named: "ic_close_new", in: bundle, compatibleWith: nil)
...
//            }
    ...
    @IBAction func libraryButtonPressed(_ sender: UIButton) {
        changeMode(FusumaMode1.library)
    }

    @IBAction func photoButtonPressed(_ sender: UIButton) {
        changeMode(FusumaMode1.camera)
    }

    @IBAction func videoButtonPressed(_ sender: UIButton) {
        changeMode(FusumaMode1.video)
    }
    ...
    private func fusumaDidFinishInSingleMode() {
        ...
        if fusumaCropImage {
            ...
            requestImage(with: self.albumView.phAsset, cropRect: cropRect) { (asset, image) in
                self.delegate?.fusuImageSelected(image, source: self.mode)
                self.doDismiss {
            ...
            delegate?.fusuImageSelected(view.image, source: mode)
    ...
    private func getMetaData(asset: PHAsset) -> ImageMetadata1 {
        let data = ImageMetadata1(mediaType: asset.mediaType ,
...
extension FusumaVC: FSAlbumViewDelegate, FSCameraViewDelegate1, FSVideoCameraViewDelegate1 {
   
    func videoButtonPressed() {
//        videoButtonPressed(UIButton())
        changeMode(FusumaMode1.video)
    }
    
    func photoButtonPressed() {
//        photoButtonPressed(UIButton())
        changeMode(FusumaMode1.camera)
    }
    ...
    func cameraShotFinished(_ image: UIImage) {
        delegate?.fusuImageSelected(image, source: mode)
        doDismiss {
            self.delegate?.fusumaDismissedWithImage(image, source: self.mode)
        }
    }
    ...
    public func albumViewCameraRollUnauthorized() {
        updateDoneButtonVisibility()
        delegate?.fusuCameraRollUnauthorized()
    }

    func videoFinishedFusu(withFileURL fileURL: URL) {
        delegate?.fusuVideoCompleted(withFileURL: fileURL)
        doDismiss(completion: nil)
    }

}

private extension FusumaVC {
    func stopAll() {
        if availableModes.contains(.video) {
    ...
    func changeMode(_ mode: FusumaMode1, isForced: Bool = false) {
    ...
    func getTabButton(mode: FusumaMode1) -> UIButton {
        switch mode {
        case .library:
